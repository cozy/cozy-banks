...
const fetchBillsForHealthExpenses = (dispatch, transactions) => {
  const docIds = uniq(flatten(transactions
...
export const fetchTransactions = exports.fetchTransactions = (options = {}, onFetch) => {
  ...
  action.promise = (client, dispatch) => {
    return originalPromise(client)
      ...
      .then(res => {
        if (onFetch) { onFetch(dispatch, res) }
        if (firstFetch) {
          firstFetch = false
...
export const fetchTransactionsWithState = exports.fetchTransactionsWithState = onFetch => (dispatch, getState) => {
  const state = getState()
  const accountIds = getFilteredAccountIds(state)
  const period = getPeriod(state)
  const month = period && period.length === 8 && period
  assert(!month || (month && monthRx.exec(month)), 'month must be in the form YYYY-MM')
  ...
  return dispatch(exports.fetchTransactions({
